...
function simplex_tableau(A, b, c, Basis, rule)
    ...
    while !all(x -> x <= 0, r)
        ...
        elseif rule == 2 #Bland's rule (smallest index in r with a positive coefficient)
            positive_indices_in_r = findall(x -> x > 0, r)
            if size(positive_indices_in_r)[1] > 0
                in_index = positive_indices_in_r[1]
            else
                in_index = 1
            end
